...
4)Used The prettytable library to out my code in a much more presentable manner.
"""
import time
import requests
...
def statewise(states,ch):
    ...
    t=PrettyTable(['State Name','Active Cases','Cured/Discharges','Deaths','Total Confirmed Cases'])
    t.add_row([stt[1],stt[2],stt[3],stt[4],stt[5]])
    print(t)
    time.sleep(1)
...
def totalcases(t1):
    ...
    t = PrettyTable(['Active Cases', 'Cured/Discharges', 'Deaths', 'Total Confirmed Cases'])
    t.add_row([first1[1], total[1], total[2], total[3]])
    print(t)
    time.sleep(1)
